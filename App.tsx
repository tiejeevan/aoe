...
const App: React.FC = () => {
    ...
    }, [units.villagers, currentAge, addToLog, updateResources, setActivityStatus, addNotification]);
    ...
    const handleAssignVillagersToNode = (nodeId: string, count: number) => {
        ...
        // 1. Calculate the next state of the node deterministically.
        const finalNodeState: ResourceNode = {
            ...targetNode,
            assignedVillagers: [...new Set([...targetNode.assignedVillagers, ...villagerIdsToAssign])]
        };
    
        // 2. Update the full list of resource nodes in state.
        setResourceNodes(prevNodes => 
            prevNodes.map(n => n.id === nodeId ? finalNodeState : n)
        );
    
        // 3. Continue with the logic using the deterministic `finalNodeState`.
